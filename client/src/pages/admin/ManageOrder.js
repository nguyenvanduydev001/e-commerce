          ...
          <tbody>
            {orders?.map((el, idx) => (
              <tr className="border-b border-gray-300 hover:" key={el._id}>
                ...
                <td className="text-center py-2">
                  {convertUSDToVND(el.total).toLocaleString("vi-VN")} VND
                </td>
                <td className="text-center py-2">{el.status}</td>
                <td className="text-center py-2">{formatDate(el.createdAt)}</td>
                <span
                  className={`inline-block text-center mt-[12px] py-2 px-4 rounded cursor-pointer ${
                    accepted ? "bg-blue-500 text-white" : "bg-main text-white"
                  }`}
                  onClick={handleClick}
                >
                  {accepted ? "Accept" : "Waiting..."}
                </span>
              </tr>
            ))}
          </tbody>
